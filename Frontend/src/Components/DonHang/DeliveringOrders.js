...
import { GetDeliveringOrders } from '../../features/invoice/invoiceSlice';

const DeliveringOrders = () => {
  const authState = useSelector(state => state?.auth?.user?.userDto);
  const invocieState = useSelector(state => state?.invoice?.delivering_invoices);
  const dispatch = useDispatch()
  useEffect(() => {
    dispatch(GetDeliveringOrders(authState?.id))
  }, [])
  return (
    <div>
      {
        !invocieState || invocieState.length === 0 ? (
          <div className='bg-light shadow pt-4 pb-4 mb-3 bg-white rounded d-flex justify-content-center align-items-center' style={{ height: '600px' }}>
            <img src='logo_donhang.png' alt='Invoice' style={{ width: '100px' }} />
          </div>
        ) : (
          invocieState.map((item, index) => (
