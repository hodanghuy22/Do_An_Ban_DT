...
import { GetCancelInvoice } from '../../features/invoice/invoiceSlice';

const CancelledOrders = () => {
  const authState = useSelector(state => state?.auth?.user?.userDto);
  const invocieState = useSelector(state => state?.invoice?.cancel_invoices);
  const dispatch = useDispatch()
  useEffect(() => {
    dispatch(GetCancelInvoice(authState?.id))
  }, [])
  return (
    <div>
      {
