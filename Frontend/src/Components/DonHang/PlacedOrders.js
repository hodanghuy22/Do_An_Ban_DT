...
import { GetPlacedOrders } from '../../features/invoice/invoiceSlice';

const PlacedOrders = () => {
  const authState = useSelector(state => state?.auth?.user?.userDto);
  const invocieState = useSelector(state => state?.invoice?.place_invoices);
  const dispatch = useDispatch()
  useEffect(() => {
    dispatch(GetPlacedOrders(authState?.id))
  }, [])
  return (
    <div>
      {
