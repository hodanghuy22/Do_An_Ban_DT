...
import { GetDeliveredOrders } from '../../features/invoice/invoiceSlice';

const DeliveredOrders = () => {
  const authState = useSelector(state => state?.auth?.user?.userDto);
  const invocieState = useSelector(state => state?.invoice?.delivered_invoices);
  const dispatch = useDispatch()
  useEffect(() => {
    dispatch(GetDeliveredOrders(authState?.id))
  }, [])
  return (
    <div>
      {
