...
import { GetConfirmedOrders } from '../../features/invoice/invoiceSlice';

const ConfirmedOrders = () => {
  const authState = useSelector(state => state?.auth?.user?.userDto);
  const invocieState = useSelector(state => state?.invoice?.comfirm_invoices);
  const dispatch = useDispatch()
  useEffect(() => {
    dispatch(GetConfirmedOrders(authState?.id))
  }, [])
